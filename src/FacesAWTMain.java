...
public class FacesAWTMain {
	...
	class FaceFrame extends Frame {
		...
		public void paint(Graphics g) {
			...
			for (int i = 0; i < 3; i++) {
				for (int a = 0; a < 3; a++) {
					fobjs[a + i].setPosition(xStart0 + a * 250, yStart0 + i * 250);
					fobjs[a + i].drawFace(g);
				}
			}
		}
	...
	private class FaceObj {
		...
		public void drawEye(Graphics g, int r) { // rは目の半径
			...
			g.drawOval(xStart + 45 - a / 2, yStart + 65, r + a, r + a);
			g.drawOval(xStart + 125 - a / 2, yStart + 65, r + a, r + a);
